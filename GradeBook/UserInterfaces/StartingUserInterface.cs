...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            CreateGradeBook(name, weight);
            Console.WriteLine("Created gradebook {0}.", name);
        }

        private static void CreateGradeBook(string type, bool weight)
        {
            string convertedType = char.ToUpperInvariant(type[0]) + type.Substring(1);
            if (convertedType == GradeBookType.Standard.ToString())
            {
                StandardGradeBook gradeBook = new StandardGradeBook(type, weight);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else if (type == GradeBookType.Standard.ToString())
            {
                RankedGradeBook gradeBook = new RankedGradeBook(type, weight);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else
            {
